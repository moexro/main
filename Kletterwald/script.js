      ...
      (function () {
        ...
        document.querySelectorAll("[data-reset-checklist]").forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            const state = loadChecklistState();
            panel.querySelectorAll(".check-item.toggleable[data-id]").forEach((el) => {
      ...
      function renderPaket3Modal() {
        ...
        paket3ModalBody.innerHTML = PAKET3_CHECKLIST.map((section) => {
          const rows = section.items
            .map((label) => {
              ...
              const on = !!state[id];
              return `
          <div class="check-item toggleable ${on ? "on" : "off"}" data-id="${id}">
        ...
        paket3ModalBody.querySelectorAll(".check-item.toggleable[data-id]").forEach((el) => {
          el.addEventListener("click", () => {
            const nowOn = !el.classList.contains("on");
            applyCheckItemVisual(el, nowOn);
      ...
      document.getElementById("paket3ModalClose").addEventListener("click", closePaket3Modal);
      ...
      (function () {
        ...
        async function loadWeather(selectedDate, openHours) {
          ...
          try {
            ...
            const hourStartIdx = data.hourly.time.findIndex((t) =>
              t.startsWith(ds),
            );
